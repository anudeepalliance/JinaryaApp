...
import static com.example.android.jinarya.RelationshipNameActivity.guestGender;
import static com.example.android.jinarya.RelationshipNameActivity.relationshipGuestAge;
import static com.example.android.jinarya.RelationshipNameActivity.relationshipGuestGender;
import static com.example.android.jinarya.RelationshipNameActivity.relationshipGuestName;
import static com.example.android.jinarya.RelationshipNameActivity.relationshipGuestViewName;
import static com.example.android.jinarya.RelationshipNameActivity.relationshipOwnerAge;
import static com.example.android.jinarya.RelationshipNameActivity.relationshipOwnerGender;
...
public class GuestNamesListActivity extends AppCompatActivity
        implements GuestNamesListAdapter.GuestNamesListItemClickListener,
        GuestNamesListAdapter.GuestNamesListItemLongClickListener{
    ...
    private GuestNamesListAdapter mAdapter;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        cursor = getAllGuestNames();

        mAdapter = new GuestNamesListAdapter(cursor,
                this, this);
    ...
    public void onListItemClick(long namesId) {
        ...
        relationshipNameActivityIntent.putExtra(relationshipViewName, relationshipGuestViewName);

        relationshipNameActivityIntent.putExtra(relationshipGuestName, name.getName());
        relationshipNameActivityIntent.putExtra(relationshipGuestAge, ageSelection);

        relationshipNameActivityIntent.putExtra(relationshipOwnerName,
                intent.getStringExtra(relationshipOwnerName));
        relationshipNameActivityIntent.putExtra(relationshipOwnerAge,
                intent.getIntExtra(relationshipOwnerAge, 0));

        relationshipNameActivityIntent.putExtra(relationshipOwnerGender, intent.getStringExtra(relationshipOwnerGender));
        relationshipNameActivityIntent.putExtra(relationshipGuestGender, intent.getStringExtra(relationshipGuestGender));
    ...
    private Cursor getAllGuestNames() {

        String mGuestGender = guestGender;

        String selection = NamesContract.NamesEntry.COLUMN_GENDER + " = ?";

        String[] guestGender = {mGuestGender};
    ...
    private void deleteName(long namesIdId) {
        ...
        cursor = getAllGuestNames();
        mAdapter.swapGuestNamesCursor(cursor);
